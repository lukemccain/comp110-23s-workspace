...
def emojified(guess: str, secret: str) -> str:
    ...
    while i < len(secret):
        if guess[i] == secret[i]:
            ...
            while not character_exist and alternate < len(secret):
                if guess[i] == secret[alternate]:
                    character_exist: bool = True
                    s = s + YELLOW_BOX
                else:
                    alternate = alternate + 1
            if character_exist == False:
                s = s + WHITE_BOX
        i = i + 1
    return s
...
def main() -> None:
    ...
    turns: int = 1
    success: bool = False
    while turns < 7 and success == False:
